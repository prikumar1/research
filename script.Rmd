...
# create csv files for fasttext training 
write.csv(emotion_keywords, "word_tokens/emotion_keywords.csv")
write.csv(emotion_abstracts, "word_tokens/emotion_abstracts.csv")
write.csv(cooperation_keywords, "word_tokens/cooperation_keywords.csv")
write.csv(cooperation_abstracts, "word_tokens/cooperation_abstracts.csv")
```
...
skm_ck1 <- skmclustering1(160, ck_vectors)
skm_ca1 <- skmclustering1(300, ca_vectors)
skm_ek1 <- skmclustering1(230, ek_vectors) 
skm_ea1 <- skmclustering1(240, ea_vectors)
...
# Apply function to add column with cluster number to matrix
#skm_ck2 <- skmclustering2(nclusters_ck2, ck_vectors)
#skm_ca2 <- skmclustering2(nclusters_ca2, ca_vectors)
#skm_ek2 <- skmclustering2(nclusters_ek2, ek_vectors) 
#skm_ea2 <- skmclustering2(nclusters_ea2, ea_vectors)
...
```{r}
emotion_keywords <- readRDS("word_tokens/emotion_keywords.RData")
skm_ek1 <- readRDS("skmean_clusters/skm_ek1.RData")
```
...
# apply function
example <- data_tables(emotion_keywords, skm_ek1)
example
```
...
# get unique phenomena in each doc_id
  ...
  most_common_20 <- head(sort$cluster, 20)
...
x <- example[example$cluster == 10,]
v <- select(x, cluster, word, freq, labels)

data_count <- v %>% 
    group_by(labels, word) %>%
    summarise(count = sum(freq))
data_count <-  data_count[order(-data_count$count),]
...
example2 <- select(example, cluster, word, freq, labels)

example2 <- example2 %>% 
    group_by(labels, word) %>%
    summarise(count = sum(freq))
example2 <-  example2[order(-example2$count),]
example2[example2$labels == "emotion",]
```
...
network <- function(co_occurence){
  ...
  visNetwork(nodes, edges, width = "100%") %>% 
    ...
    visEdges(aes(width = width, edge_alpha = width), color = list(highlight = "blue", hover = "blue")) %>%
    visNodes(size = 15) %>%
    visSave(file = "example_network.html")
}
network(co_occurence)
```
...
nodes <- unique(c(co_occurence$item1, co_occurence$item2))
  ...
  visEvents(select = "function(nodes) {
    ...
    output$table <- renderTable(example2[example2$labels == current_node_selection,]  %>% 
    edge %>% 
...
server <- function(input, output) {
  output$table <- renderTable(example2)
